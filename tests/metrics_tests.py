...
class DistancesTest(unittest.TestCase):
    ...
    def test_euclidean(self):
        self.assertListEqual(euclidean_distance(self.A, self.B),
                             [0.9506105259861932, 1.068591055912026,
                              0.19746815659817757])

    def test_manhattan(self):
        self.assertListEqual(manhattan_distance(self.A, self.B),
                             [1.390086685264639, 1.6562536208811662,
                              0.26064691033112874])

    def test_norm3(self):
        self.assertListEqual(calculate_distance(self.A, self.B, 3),
                             [0.8460414137912776, 0.9428389207901624,
                              0.19178702833515607])

    def test_mse(self):
        self.assertAlmostEqual(mean_squared_error(
            self.regressor.predict(self.X_test), self.y_test),
            1.5470835956432736)
    ...
    def test_distance_error(self):
        self.assertRaises(ValueError, calculate_distance,
                          self.regressor.predict(self.X_test), self.y_test,
                          "the_ultimate_norm")
